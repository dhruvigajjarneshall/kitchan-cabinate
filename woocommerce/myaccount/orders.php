								...
								<?php elseif ( 'order-actions' === $column_id ) : ?>
									...
									if ( ! empty( $actions ) ) {
										foreach ( $actions as $key => $action ) {
											echo '<a href="' . esc_url( $action['url'] ) . '" class="btn btn-link woocommerce-button button ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
										}
									}
									?>
								<?php endif; ?>
...
<?php else : ?>
	...
	<div class="clb-blank">
		...
		<h4 class="clb-blank-headline">
			<?php esc_html_e( 'No order has been made yet.', 'ohio' ); ?>
		</h4>
		<a class="woocommerce-Button button btn btn-small" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
			<?php esc_html_e( 'Go Shopping', 'ohio' ); ?> <i class="ion-right ion ion-ios-arrow-forward"></i>
		</a>
	</div>
